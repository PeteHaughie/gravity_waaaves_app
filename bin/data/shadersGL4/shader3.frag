...
float dither2(float inColor, vec2 inCoord, float ditherPalette) {
  ...
  float indexValue = indexMatrix8x8[x + y * 8] * 0.015625;
  ...
  float colorDiff =
    distance(inColor, closestColor) /
    distance(secondClosestColor, closestColor);

  return colorDiff < indexValue
    ? closestColor
    : secondClosestColor;
}
...
void main() {
  ...
  if (block1HMirror == 1) {
    if (block1Coords.x > width / 2) {
      block1Coords.x = abs(width - block1Coords.x);
    }
  } //endifhflip1
  if (block1VMirror == 1) {
    if (block1Coords.y > height / 2) {
      block1Coords.y = abs(height - block1Coords.y);
    }
  } //endifvflip1

  block1Coords = kaleidoscope(
    block1Coords,
    block1KaleidoscopeAmount,
    block1KaleidoscopeSlice
  );
  ...
  if (block1GeoOverflow == 1) {
    block1Coords = wrapCoord(block1Coords);
  }
  if (block1GeoOverflow == 2) {
    block1Coords = mirrorCoord(block1Coords);
  }

  vec4 block1Color = blurAndSharpen(
    block1Output,
    block1Coords,
    block1SharpenAmount,
    block1SharpenRadius,
  ...
  if (block1GeoOverflow == 0) {
    if (
      block1Coords.x > 1280.0 ||
      block1Coords.y > 720.0 ||
      block1Coords.x < 0.0 ||
      block1Coords.y < 0.0
    ) {
  ...
  if (block1ColorizeSwitch == 1) {
    ...
    if (block1ColorizeHSB_RGB == 0) {
      //assign bands
      vec3 colorizedHSB1 =
        block1ColorizeBand1 + vec3(0.0, 0.0, block1ColorHSB.z);
      vec3 colorizedHSB2 =
        block1ColorizeBand2 + vec3(0.0, 0.0, block1ColorHSB.z);
      vec3 colorizedHSB3 =
        block1ColorizeBand3 + vec3(0.0, 0.0, block1ColorHSB.z);
      vec3 colorizedHSB4 =
        block1ColorizeBand4 + vec3(0.0, 0.0, block1ColorHSB.z);
      vec3 colorizedHSB5 =
        block1ColorizeBand5 + vec3(0.0, 0.0, block1ColorHSB.z);
    ...
    if (block1ColorHSB.z < 0.25) {
      colorizedRGB = mix(colorizedRGB1, colorizedRGB2, block1ColorHSB.z * 4.0);
    }
    if (block1ColorHSB.z > 0.25 && block1ColorHSB.z < 0.5) {
      colorizedRGB = mix(
        colorizedRGB2,
        colorizedRGB3,
        (block1ColorHSB.z - 0.25) * 4.0
      );
    }
    if (block1ColorHSB.z > 0.5 && block1ColorHSB.z < 0.75) {
      colorizedRGB = mix(
        colorizedRGB3,
        colorizedRGB4,
        (block1ColorHSB.z - 0.5) * 4.0
      );
    }
    if (block1ColorHSB.z > 0.75) {
      colorizedRGB = mix(
        colorizedRGB4,
        colorizedRGB5,
        (block1ColorHSB.z - 0.75) * 4.0
      );
    }
  ...
  if (block2HMirror == 1) {
    if (block2Coords.x > width / 2) {
      block2Coords.x = abs(width - block2Coords.x);
    }
  } //endifhflip1
  if (block2VMirror == 1) {
    if (block2Coords.y > height / 2) {
      block2Coords.y = abs(height - block2Coords.y);
    }
  } //endifvflip1

  block2Coords = kaleidoscope(
    block2Coords,
    block2KaleidoscopeAmount,
    block2KaleidoscopeSlice
  );
  ...
  if (block2GeoOverflow == 1) {
    block2Coords = wrapCoord(block2Coords);
  }
  if (block2GeoOverflow == 2) {
    block2Coords = mirrorCoord(block2Coords);
  }

  vec4 block2Color = blurAndSharpen(
    block2Output,
    block2Coords,
    block2SharpenAmount,
    block2SharpenRadius,
  ...
  if (block2GeoOverflow == 0) {
    if (
      block2Coords.x > 1280.0 ||
      block2Coords.y > 720.0 ||
      block2Coords.x < 0.0 ||
      block2Coords.y < 0.0
    ) {
  ...
  if (block2ColorizeSwitch == 1) {
    ...
    if (block2ColorizeHSB_RGB == 0) {
      //assign bands
      vec3 colorizedHSB1 =
        block2ColorizeBand1 + vec3(0.0, 0.0, block2ColorHSB.z);
      vec3 colorizedHSB2 =
        block2ColorizeBand2 + vec3(0.0, 0.0, block2ColorHSB.z);
      vec3 colorizedHSB3 =
        block2ColorizeBand3 + vec3(0.0, 0.0, block2ColorHSB.z);
      vec3 colorizedHSB4 =
        block2ColorizeBand4 + vec3(0.0, 0.0, block2ColorHSB.z);
      vec3 colorizedHSB5 =
        block2ColorizeBand5 + vec3(0.0, 0.0, block2ColorHSB.z);
    ...
    if (block2ColorHSB.z < 0.25) {
      colorizedRGB = mix(colorizedRGB1, colorizedRGB2, block2ColorHSB.z * 4.0);
    }
    if (block2ColorHSB.z > 0.25 && block2ColorHSB.z < 0.5) {
      colorizedRGB = mix(
        colorizedRGB2,
        colorizedRGB3,
        (block2ColorHSB.z - 0.25) * 4.0
      );
    }
    if (block2ColorHSB.z > 0.5 && block2ColorHSB.z < 0.75) {
      colorizedRGB = mix(
        colorizedRGB3,
        colorizedRGB4,
        (block2ColorHSB.z - 0.5) * 4.0
      );
    }
    if (block2ColorHSB.z > 0.75) {
      colorizedRGB = mix(
        colorizedRGB4,
        colorizedRGB5,
        (block2ColorHSB.z - 0.75) * 4.0
      );
    }
  ...
  vec3 scaleVec = vec3(0.33, 0.33, 0.33);
  ...
  if (matrixMixType == 4) {
    matrixMixOut.r = dot(
      mix(fgR, bg / (1.00001 - fgR), bgRGBIntoFgRed),
      scaleVec
    );
    matrixMixOut.g = dot(
      mix(fgG, bg / (1.00001 - fgG), bgRGBIntoFgGreen),
      scaleVec
    );
    matrixMixOut.b = dot(
      mix(fgB, bg / (1.00001 - fgB), bgRGBIntoFgBlue),
      scaleVec
    );
  }
