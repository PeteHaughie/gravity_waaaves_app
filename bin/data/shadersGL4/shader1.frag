...
void main() {
  ...
  if (ch1HMirror == 1) {
    if (ch1Coords.x > 320.0) {
      ch1Coords.x = abs(640 - ch1Coords.x);
    }
  } //endifhflip1
  if (ch1VMirror == 1) {
    if (ch1Coords.y > 240.0) {
      ch1Coords.y = abs(480 - ch1Coords.y);
    }
  } //endifvflip1

  ch1Coords = kaleidoscope1(
    ch1Coords,
    ch1KaleidoscopeAmount,
    ch1KaleidoscopeSlice
  );
  ...
  if (ch1GeoOverflow == 1) {
    ch1Coords = wrapCoord1(ch1Coords);
  }
  if (ch1GeoOverflow == 2) {
    ch1Coords = mirrorCoord1(ch1Coords);
  }

  //add blur and sharpen here
  vec4 ch1Color = blurAndSharpen(
    ch1Tex,
    ch1Coords,
    ch1SharpenAmount,
    ch1SharpenRadius,
  ...
  if (ch2HMirror == 1) {
    if (ch2Coords.x > 320.0) {
      ch2Coords.x = abs(640 - ch2Coords.x);
    }
  } //endifhflip1
  if (ch2VMirror == 1) {
    if (ch2Coords.y > 240.0) {
      ch2Coords.y = abs(480 - ch2Coords.y);
    }
  } //endifvflip1

  ch2Coords = kaleidoscope1(
    ch2Coords,
    ch2KaleidoscopeAmount,
    ch2KaleidoscopeSlice
  );
  ...
  if (ch2GeoOverflow == 1) {
    ch2Coords = wrapCoord1(ch2Coords);
  }
  if (ch2GeoOverflow == 2) {
    ch2Coords = mirrorCoord1(ch2Coords);
  }

  vec4 ch2Color = blurAndSharpen(
    ch2Tex,
    ch2Coords,
    ch2SharpenAmount,
    ch2SharpenRadius,
  ...
  if (fb1HMirror == 1) {
    if (fb1Coords.x > width / 2) {
      fb1Coords.x = abs(width - fb1Coords.x);
    }
  } //endifhflip1
  if (fb1VMirror == 1) {
    if (fb1Coords.y > height / 2) {
      fb1Coords.y = abs(height - fb1Coords.y);
    }
  } //endifvflip1

  fb1Coords = kaleidoscope(
    fb1Coords,
    fb1KaleidoscopeAmount,
    fb1KaleidoscopeSlice
  );
  /*
	if(fb1HMirror==1){
  ...
  if (fb1GeoOverflow == 1) {
    fb1Coords = wrapCoord(fb1Coords);
  }
  if (fb1GeoOverflow == 2) {
    fb1Coords = mirrorCoord(fb1Coords);
  }
  ...
  if (fb1SaturationInvert == 1) {
    fb1ColorHSB.y = 1.0 - fb1ColorHSB.y;
  }
  if (fb1BrightInvert == 1) {
  ...
  temporalFilter1ColorHSB.y = clamp(
    temporalFilter1ColorHSB.y * (1.0 + fb1TemporalFilter1Resonance * 0.25),
    0.0,
    1.0
  );
  temporalFilter1ColorHSB.z = clamp(
    temporalFilter1ColorHSB.z * (1.0 + fb1TemporalFilter1Resonance * 0.5),
    0.0,
    1.0
  );
  ...
  outColor = clamp(
    mix(outColor, temporalFilter1Color, fb1TemporalFilter1Amount),
    0.0,
    1.0
  );

  temporalFilter2ColorHSB.y = clamp(
    temporalFilter2ColorHSB.y * (1.0 + fb1TemporalFilter2Resonance * 0.25),
    0.0,
    1.0
  );
  temporalFilter2ColorHSB.z = clamp(
    temporalFilter2ColorHSB.z * (1.0 + fb1TemporalFilter2Resonance * 0.5),
    0.0,
    1.0
  );
  ...
  outColor = clamp(
    mix(outColor, temporalFilter2Color, fb1TemporalFilter2Amount),
    0.0,
    1.0
  );
