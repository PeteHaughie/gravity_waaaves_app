...
vec4 blurAndSharpen(
  sampler2DRect blurAndSharpenTex,
  vec2 coord,
  float sharpenAmount,
  float sharpenRadius,
  float sharpenBoost,
  ...
  float color_sharpen_bright =
    rgb2hsb(
      texture(blurAndSharpenTex, coord + vec2(sharpenRadius, 0)).rgb
    ).z +
    rgb2hsb(
      texture(blurAndSharpenTex, coord + vec2(-sharpenRadius, 0)).rgb
    ).z +
    rgb2hsb(
      texture(blurAndSharpenTex, coord + vec2(0, sharpenRadius)).rgb
    ).z +
    rgb2hsb(
      texture(blurAndSharpenTex, coord + vec2(0, -sharpenRadius)).rgb
    ).z +
    rgb2hsb(
      texture(
        blurAndSharpenTex,
        coord + vec2(sharpenRadius, sharpenRadius)
      ).rgb
    ).z +
    rgb2hsb(
      texture(
  ...
  color_sharpen_bright = color_sharpen_bright * 0.125;
...
vec2 rotate(vec2 coord, float theta, int mode) {
  ...
  if (mode == 1) {
    vec2 center_coord = vec2(
      (coord.x - width / 2) * inverseWidth,
      (coord.y - height / 2) * inverseHeight
    );
    float spiral = abs(coord.x + coord.y) / 2 * width;
    coord.x = spiral + coord.x;
    coord.y = spiral + coord.y;
...
vec2 rotate1(vec2 coord, float theta, int mode) {
  ...
  if (mode == 1) {
    vec2 center_coord = vec2(
      (coord.x - width1 / 2) * inverseWidth1,
      (coord.y - height1 / 2) * inverseHeight1
    );
    float spiral = abs(coord.x + coord.y) / 2 * width1;
    coord.x = spiral + coord.x;
    coord.y = spiral + coord.y;
...
vec4 mixnKeyVideo(
  ...
  if (chromaDistance < keyThreshold) {
    ...
    outColor = mix(
      bg,
      outColor,
      keySoft * abs(1.0 - (chromaDistance - keyThreshold))
    );
    //outColor=bg;
...
void main() {
  ...
  if (block2InputHMirror == 1) {
    if (block2InputCoords.x > block2InputWidthHalf) {
      block2InputCoords.x = abs(block2InputWidth - block2InputCoords.x);
    }
  } //endifhflip1
  if (block2InputVMirror == 1) {
    if (block2InputCoords.y > block2InputHeightHalf) {
      block2InputCoords.y = abs(block2InputHeight - block2InputCoords.y);
    }
  } //endifvflip1

  if (block2InputWidth == 1280) {
    block2InputCoords = kaleidoscope(
      block2InputCoords,
      block2InputKaleidoscopeAmount,
      block2InputKaleidoscopeSlice
    );
  }
  if (block2InputWidth == 640) {
    block2InputCoords = kaleidoscope1(
      block2InputCoords,
      block2InputKaleidoscopeAmount,
      block2InputKaleidoscopeSlice
    );
  ...
  //got to fix these...
  if (block2InputGeoOverflow == 1) {
    block2InputCoords = wrapCoord1(
      block2InputCoords,
      block2InputWidth,
      block2InputHeight
    );
  }
  if (block2InputGeoOverflow == 2) {
    block2InputCoords = mirrorCoord1(
      block2InputCoords,
      block2InputWidth,
      block2InputHeight
    );
  ...
  vec4 block2InputColor = blurAndSharpen(
    block2InputTex,
    block2InputCoords,
    block2InputSharpenAmount,
    block2InputSharpenRadius,
    block2InputFiltersBoost,
    block2InputBlurRadius,
    block2InputBlurAmount
  );
  ...
  //clamp shits out
  if (
    block2InputCoords.x > block2InputWidth ||
    block2InputCoords.y > block2InputHeight ||
    block2InputCoords.x < 0.0 ||
    block2InputCoords.y < 0.0
  ) {
  ...
  //inverts
  if (block2InputHueInvert == 1) {
    block2InputColorHSB.x = 1.0 - block2InputColorHSB.x;
  }
  if (block2InputSaturationInvert == 1) {
    block2InputColorHSB.y = 1.0 - block2InputColorHSB.y;
  }
  if (block2InputBrightInvert == 1) {
    block2InputColorHSB.z = 1.0 - block2InputColorHSB.z;
  }
  ...
  if (block2InputRGBInvert == 1) {
    block2InputColor.rgb = 1.0 - block2InputColor.rgb;
  }

  if (block2InputPosterizeSwitch == 1) {
    block2InputColor.rgb = colorQuantize(
      block2InputColor.rgb,
      block2InputPosterize,
      block2InputPosterizeInvert
    );
  }
  ...
  if (fb2HMirror == 1) {
    if (fb2Coords.x > width / 2) {
      fb2Coords.x = abs(width - fb2Coords.x);
    }
  } //endifhflip1
  if (fb2VMirror == 1) {
    if (fb2Coords.y > height / 2) {
      fb2Coords.y = abs(height - fb2Coords.y);
    }
  } //endifvflip1

  fb2Coords = kaleidoscope(
    fb2Coords,
    fb2KaleidoscopeAmount,
    fb2KaleidoscopeSlice
  );
  ...
  if (fb2GeoOverflow == 1) {
    fb2Coords = wrapCoord(fb2Coords);
  }
  if (fb2GeoOverflow == 2) {
    fb2Coords = mirrorCoord(fb2Coords);
  }
  ...
  if (fb2SaturationInvert == 1) {
    fb2ColorHSB.y = 1.0 - fb2ColorHSB.y;
  }
  if (fb2BrightInvert == 1) {
  ...
  temporalFilter1ColorHSB.y = clamp(
    temporalFilter1ColorHSB.y * (1.0 + fb2TemporalFilter1Resonance * 0.25),
    0.0,
    1.0
  );
  temporalFilter1ColorHSB.z = clamp(
    temporalFilter1ColorHSB.z * (1.0 + fb2TemporalFilter1Resonance * 0.5),
    0.0,
    1.0
  );
  ...
  outColor = clamp(
    mix(outColor, temporalFilter1Color, fb2TemporalFilter1Amount),
    0.0,
    1.0
  );

  temporalFilter2ColorHSB.y = clamp(
    temporalFilter2ColorHSB.y * (1.0 + fb2TemporalFilter2Resonance * 0.25),
    0.0,
    1.0
  );
  temporalFilter2ColorHSB.z = clamp(
    temporalFilter2ColorHSB.z * (1.0 + fb2TemporalFilter2Resonance * 0.5),
    0.0,
    1.0
  );
  ...
  outColor = clamp(
    mix(outColor, temporalFilter2Color, fb2TemporalFilter2Amount),
    0.0,
    1.0
  );
